		...
		mysql)
			if [[ $mysql_useable -ne 1 ]]; then
				if [[ $dist_codename == "xenial" ]]; then
					mysql --defaults-file=/etc/mysql/debian.cnf -e "SET CREDENTIALS for 'root' to '$my_rootpw'; FLUSH PRIVILEGES;"
...
DatabaseMirror clamav.inode.at" >> /etc/clamav/freshclam.conf
		...
		webserver)
			...
			if [[ ${httpd_platform} == "nginx" ]]; then
				...
				cp webserver/nginx/conf/sites-available/mailcow_php${php_version} /etc/nginx/sites-available/mailcow
				...
				ln -s /etc/nginx/sites-available/mailcow_php${php_version} /etc/nginx/sites-enabled/000-0-mailcow 2>/dev/null
